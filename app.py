...
# Пытаемся подключиться к PostgreSQL, если не получается - используем SQLite
try:
    ...
    if DB_HOST and DB_NAME and DB_USER and DB_PASSWORD:
        ...
        print("⚠️ Используется SQLite (нет переменных PostgreSQL)")
except Exception as e:
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///tabakmen.db'
    print(f"⚠️ Ошибка подключения к PostgreSQL, используется SQLite: {e}")
    ...
    try:
        ...
        if 'photo' in request.files:
            ...
            if photo and photo.filename:
                ...
                print(f"✅ Фото сконвертировано в base64")
